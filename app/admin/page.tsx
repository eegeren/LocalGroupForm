...
export default function AdminPage() {
  const [authed, setAuthed] = useState(true) // middleware'i kaldırdığımız için direkt true
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((it:any) =>
      [it.fullName, it.phone, it.subject, it.message, it.positionApplied, it.prevCompany, it.prevTitle]
  ...
  return (
    <main className="max-w-5xl mx-auto p-6 space-y-4">
      ...
      {err && <div className="p-3 rounded-lg border border-red-300 bg-red-50 text-red-700 text-sm">{err}</div>}
      ...
      <div className="bg-white rounded-2xl shadow border border-neutral-200 divide-y divide-neutral-200">
        {filtered.map((it:any) => (
          <div key={it.id} className="py-4 px-4">
            ...
            <div className="mt-2 text-neutral-800">Pozisyon: {it.positionApplied || '-'} | Tür: {it.employmentType || '-'}</div>
            <div className="text-sm text-neutral-600">Vardiya: {it.shiftAvailability || '-'}</div>

            <div className="mt-2 text-neutral-800">Eğitim: {it.educationLevel || '-'}</div>
            <div className="text-sm text-neutral-600">Yabancı Dil: {it.foreignLanguages || '-'}</div>
            ...
            <div className="mt-1 text-xs text-neutral-500">
              KVKK: {it.consent ? 'Evet' : 'Hayır'} {it.consentAt ? `(${new Date(it.consentAt).toLocaleString('tr-TR')})` : ''}
